    ...
    public partial class XCProject : System.IDisposable
    {
        ...
        public void ApplyMod(XCMod mod)
        {
            ...
            foreach (XCModFile libRef in mod.libs)
            {
                ...
                AddFile(completeLibPath, modGroup, "SDKROOT", true, libRef.isWeak);
            }

            Debug.Log("Adding frameworks...");
            PBXGroup frameworkGroup = GetGroup("Frameworks");
            foreach (string framework in mod.frameworks)
            {
                string[] filename = framework.Split(':');
                bool isWeak = (filename.Length > 1) ? true : false;
                string completePath = Path.Combine("System/Library/Frameworks", filename[0]);
                AddFile(completePath, frameworkGroup, "SDKROOT", true, isWeak);
            }
            ...
            foreach (string filePath in mod.files)
            {
                ...
                AddFile(absoluteFilePath, modGroup);
            }

            Debug.Log("Adding folders...");
            foreach (string folderPath in mod.folders)
            {
                string absoluteFolderPath = Path.Combine(mod.path, folderPath);
            ...
            foreach (string headerpath in mod.headerpaths)
            {
                ...
                AddHeaderSearchPaths(absoluteHeaderPath);
            }
            Consolidate();
        }
