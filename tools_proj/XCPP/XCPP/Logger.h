...
public:
	Logger(LogLevel level) : m_log_rank(level) {};  
	~Logger(void);
...
// 根据不同等级进行用不同的输出流进行读写  
#define LOG(text) Logger(INFO).Start(INFO, text,__LINE__,__FUNCTION__)  

#define WARN(text) Logger(WARN).Start(WARN,text,__LINE__,__FUNCTION__)

#define ERROR(text) Logger(ERROR).Start(ERROR,text,__LINE__,__FUNCTION__)
