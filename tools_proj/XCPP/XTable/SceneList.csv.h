...
class SceneList.csv:public NativeReader
{
public:
	SceneList.csv(void);
	void ReadTable();
	void GetRow(int val,SceneList.csvRow* row);
	int GetLength();

protected:
	std::string name;
	std::vector<SceneList.csvRow> m_data;
};


extern "C"
{
	ENGINE_INTERFACE_EXPORT int iGetSceneList.csvLength();
	ENGINE_INTERFACE_EXPORT void iGetSceneList.csvRow(int suitid,SceneList.csvRow* row);
};